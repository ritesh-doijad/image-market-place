...
const PhotosPurchased = () => {
  ...
  const getMyPosts = async () => {
    try {
      ...
      const q = query(productsRef, where("purchasedBy", "array-contains", userId));
  ...
  useEffect(() => {
    if (posts.length === 0) {
      ...
      setPostsFound(true); // If posts are already in state, set postsFound to true
    }
  }, [posts.length]);
  ...
  return (
    <div>
      ...
      <div className="mx-8">
        {loading && <p>Loading...</p>}
        {!loading && !postsFound && <p className="text-center text-lg">No images purchased.</p>}
        <div className={`grid md:grid-cols-3 gap-4 overflow-y-auto scrollbar-hide ${postsFound ? "" : "hidden"}`} style={{ maxHeight: "550px" }}>
